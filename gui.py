...
from utils import (
    ...
    load_config,
    save_config,
    write_log,         # <–– добавили write_log
    load_blacklist     # <–– чтобы при сборе ссылок можно было загружать blacklist
)
...
def create_gui():
    ...
    def create_blacklist_process():
        """
        Раньше тут стояли print(), теперь вместо них – write_log,
        чтобы сообщения шли в текстовое поле лога GUI.
        """
        total_blacklist = set()
        ...
        for mode in modes:
            page = 0
            while True:
                # Блокировка/пауза, если пользователю нужно приостановить
        ...
        try:
            # Запись итогового файла blacklist.txt
            ...
            messagebox.showerror("Ошибка", f"Не удалось записать blacklist.txt: {e}")
